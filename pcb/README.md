...
| CAD mechanical review | PASS for fabrication-file preparation; enclosure rebuilt 2026-09-18 | the board outline or battery envelope changes |
...
*Enclosure.* Rebuilt on 2026-09-18 after three structural defects were found in the 2026-09-17/18
geometry (see the log). The board is now mechanically captured: it sits on a 1.25 mm perimeter ledge
in the tray with 1.00 mm of bearing, is held down by full-length lid blocks on both long edges with a
0.05 mm nominal gap, and has 0.25 mm of lateral slop per side instead of 0.50. The halves join on a
continuous 2.445 mm lap joint whose four snap fingers are rooted in the lid wall over their full
6.0 x 0.7 mm section. Closed exterior is now **27.7 x 49.7 x 12.75 mm**. CAD review passes with zero
modeled hard collisions, 2.1 mm battery-to-antenna clearance, a single-piece USB mouth, separate
MAX30101 optical-gasket and TMP117 soft-thermal-pad volumes, and an unobstructed SHT40 vent.
...
| 2026-09-18 | enclosure | **Structural rebuild — three defects found and fixed in the snap-fit revision** | **PASS in CAD; physical print and fit test still open.** Audit of the 2026-09-18 snap-fit geometry found three defects that would have made a printed part useless. (1) Each of the four snap tabs was joined to the lid over only **0.10 mm of height, 0.18 mm^3** of weld: the tab spanned z -2.05..+0.25 while the lid wall began at z +0.15, so a 2.2 mm cantilever hung from a sliver thinner than one FDM layer. (2) **The PCB was not retained at all** — the tray cavity was a plain 25 x 47 x 1.85 mm pocket with no ledge, standoff or hold-down, leaving the board floating with 0.50 mm of lateral slop and 1.05 mm of vertical travel. (3) The parting line at z +0.15 ran 0.195 mm *below* the board's top face, so the tray rim could never capture the board edge. `enclosure/generate_enclosure.py` was rewritten around a measured board datum (STEP bodies: board face -0.085..+0.995, top-side bodies from +0.995, lowest bottom-side body -0.905; shifted by -0.65). New structure: parting plane on the board's top face; tray carries a 1.25 mm perimeter ledge (1.00 mm bearing) plus four blind snap catches; lid carries a continuous 0.70 mm lap skirt 2.445 mm deep, four 6.0 x 0.70 mm snap fingers rooted over their full section (**2.10 mm^3**, 11.7x the old weld) with 0.20 mm hooks, full-length PCB hold-down blocks on both long edges (2.30 mm west / 1.00 mm east overlap, 0.05 mm gap), battery shelves bearing 0.55 mm west and 0.70 mm east, and a 0.60 mm battery end stop immediately north of the cell so it cannot drift over the trace antenna. Footprint tightened to **27.7 x 49.7 mm** (1.6 mm wall, 0.25 mm slop per side); height is **12.75 mm**, up 0.35 mm, to buy lap-joint depth and a 0.72 mm floor under the joint groove. Peak snap bending strain is **3.51%** by eps = 1.5*t*delta/L^2, inside PETG's short-term allowable; PLA is unsuitable. Verification: `enclosure/generate_enclosure.py` run under `.venv` (CadQuery 2.8.0, installed this session), exit 0, **24/24 checks pass**; all 12 collision volumes 0.0000 mm^3 including hard shell vs populated PCB, shell vs battery and assembled top vs bottom. Independent STL check: both parts are **1 connected component with 0 non-manifold edges**. Evidence: `enclosure/generated/mechanical_review/mechanical_review.json`, regenerated STEP/STL/SVG. **This supersedes the 2026-09-17 and earlier 2026-09-18 enclosure rows.** Two defects the rebuild does not remove and the first print must check: the battery end stop bridges 13.2 mm over the ESP32-C3-MINI-1 when the lid prints roof-down, and the USB-C mouth is sized to the receptacle envelope, not to a specific cable overmold. |
...
2. **Print and fit-test the enclosure** with the real PCB, the EEMB cell, the optical gasket and the
   ...
   `enclosure/generated/mechanical_review/`. Print in PETG or nylon, not PLA. Specifically check the
   0.10 mm lap-joint clearance per face, the 0.20 mm snap-hook engagement, the 0.05 mm PCB hold-down
   gap, the 13.2 mm roof-down bridge over the ESP32 module, and that a real USB-C cable's overmold
